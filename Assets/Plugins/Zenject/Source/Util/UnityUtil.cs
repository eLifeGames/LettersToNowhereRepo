...
namespace ModestTree.Util
{
    public static class UnityUtil
    {
        ...
        public static GameObject GetRootParentOrSelf(GameObject gameObject)
            => GetParentsAndSelf(gameObject.transform).Select(x => x.gameObject).LastOrDefault();
        ...
        public static IEnumerable<GameObject> GetAllGameObjects()
            => GameObject.FindObjectsByType<Transform>(FindObjectsSortMode.None).Select(x => x.gameObject);

        public static List<GameObject> GetAllRootGameObjects()
            => GetAllGameObjects().Where(x => x.transform.parent == null).ToList();
    }
}
#endif
